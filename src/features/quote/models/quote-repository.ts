...
import { orderRepository } from '../../order/models/order-model';
import { canEditQuote, canManageQuote, quoteToday, requiresQuoteApproval, validateQuoteContent, quoteTotals } from './quote-policy'
...
export function createQuoteRepository(deps = { customers: customerRepository, products: productRepository, opportunities: opportunityRepository, orders: orderRepository }, now = () => new Date()) {
  let quotes: Quote[] = freeze([])
  let sequence = 0
  const listeners = new Set<() => void>()
  const notify = () => listeners.forEach(listener => listener())
  const audit = (version: number, actor: QuoteActor | null, action: string, detail = ''): QuoteAudit => ({ id: crypto.randomUUID(), at: now().toISOString(), actorId: actor?.id, actorName: actor?.name, action, version, detail })
  ...
  function commit(quote: Quote) {
    ...
    quotes = freeze(quotes.some(item => item.id === stored.id) ? quotes.map(item => item.id === stored.id ? stored : item) : [stored, ...quotes])
    notify()
    return stored
  }
  function expire() {
    ...
    if (changed) { quotes = freeze(next); notify() }
  }
  ...
  function content(input: QuoteContent, previous?: QuoteVersion): QuoteContent {
    const value: QuoteContent = structuredClone({
      name: input.name, customerId: input.customerId, opportunityId: input.opportunityId,
      validUntil: input.validUntil, lines: input.lines, paymentTerms: input.paymentTerms,
      deliveryTerms: input.deliveryTerms, warranty: input.warranty, notes: input.notes,
    })
  ...
  function change(quote: Quote, version: QuoteVersion, actor: QuoteActor | null, action: string, patch: Partial<QuoteVersion>, detail = '') {
  ...
  return {
    ...
    expire,
    removeMany(ids: string[], actor: QuoteActor | null) {
      role(actor, 'sales')
      const selected = new Set(ids)
      const next = quotes.filter(quote => !selected.has(quote.id))
      if (next.length === quotes.length) return
      quotes = freeze(next)
      notify()
    },
    create(input: QuoteContent, actor: QuoteActor | null) {
      role(actor, 'sales')
    ...
    update(id: string, versionId: string, expectedRevision: number, input: QuoteContent, actor: QuoteActor | null) {
      role(actor, 'sales')
    ...
    newVersion(id: string, versionId: string, actor: QuoteActor | null) {
      role(actor, 'sales')
      const { quote, version } = current(id, versionId)
      if (quote.orderId || version.status === 'Accepted') throw new Error('客戶已接受的報價不可新增版本。')
      if (version.status === 'Draft' && version.approval === 'Pending') throw new Error('請等待審批完成後再建立版本。')
      const requiresReapproval = version.approval !== 'NotRequired' || !!version.requiresReapproval
      const next: QuoteVersion = {
        ...structuredClone(version), id: crypto.randomUUID(), version: version.version + 1, revision: 0,
        status: 'Draft', requiresReapproval,
        approval: requiresReapproval || requiresQuoteApproval(version) ? 'Required' : 'NotRequired',
        createdAt: now().toISOString(), createdBy: actor?.name,
        validUntil: version.validUntil < quoteToday(now()) ? quoteToday(now()) : version.validUntil,
        sentAt: undefined, decisionAt: undefined, decisionBy: undefined, decisionReason: undefined,
        approvalAt: undefined, approvalBy: undefined, approvalReason: undefined,
      }
      const detail = `複製自 v${version.version}${next.approval === 'Required' ? '；需重新審批' : ''}`
      return commit({ ...quote, versions: [...quote.versions, next], audit: [...quote.audit, audit(next.version, actor, '建立新版本', detail)] })
    },
    requestApproval(id: string, versionId: string, actor: QuoteActor | null) {
      role(actor, 'sales')
      const { quote, version } = current(id, versionId)
    ...
    review(id: string, versionId: string, approved: boolean, reason: string, actor: QuoteActor | null) {
      role(actor, 'manager')
      const { quote, version } = current(id, versionId)
    ...
    send(id: string, versionId: string, actor: QuoteActor | null) {
      role(actor, 'sales')
      const { quote, version } = current(id, versionId)
    ...
    decide(id: string, versionId: string, accepted: boolean, reason: string, actor: QuoteActor | null) {
      role(actor, 'customer')
      const { quote, version } = current(id, versionId)
    ...
    convertToOrder(id: string, versionId: string, actor: QuoteActor | null) {
      role(actor, 'sales')
      const { quote, version } = current(id, versionId)
