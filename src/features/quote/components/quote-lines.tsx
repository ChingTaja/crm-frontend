      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm">
        ...
        <tbody>{editor.draft.lines.map((line, index) => <tr key={line.id}>
          <td className="min-w-56 border-b px-2 py-3">
            {editor.editable ? <Lookup label={`第 ${index + 1} 筆商品`} value={line.productId} options={products.filter(p => p.status === '啟用' || p.id === line.productId).map(p => ({ value: p.id, label: p.id === line.productId ? line.productName : p.name, keywords: p.sku }))} onValueChange={id => { const product = products.find(p => p.id === id); if (product) editor.selectProduct(line.id, product) }} /> : <p>{line.productName}</p>}
