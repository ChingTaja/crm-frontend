...
export function CustomerTable({ children }: ComponentProps<'table'>) {
  return <div className="overflow-x-auto"><table className="w-full border-collapse text-left whitespace-nowrap">{children}</table></div>
}

export function CustomerTableRow({ className, ...props }: ComponentProps<'tr'>) {
  return <tr className={cn('transition-colors hover:bg-[#fafbf8] data-[selected=true]:bg-[#f0f5ec]', className)} {...props} />
}

export function CustomerTableHead({ selection, className, ...props }: ComponentProps<'th'> & { selection?: boolean }) {
  return <th className={cn(cellClasses, 'h-12 py-2.5 text-xs text-[#939b90]', selection && selectionClasses, className)} {...props} />
}
