...
export function SalesSidebar({ entity }: { entity: string }) {
  ...
  return <aside className="border-r bg-[#f8f9f8] p-4">
    ...
    <nav className="flex flex-wrap gap-1 md:grid">{links.map(({ key, label, english, icon: Icon }) => <a key={key} href={`#/${key}`} aria-current={entity === key ? 'page' : undefined} className="flex flex-1 items-center gap-2 rounded-lg p-3 text-muted-foreground hover:bg-muted aria-[current=page]:bg-[#e9eee7] aria-[current=page]:text-[#284c36]"><Icon size={18} />{label}<span className="ml-auto text-[10px]">{english}</span></a>)}</nav>
  </aside>
}
