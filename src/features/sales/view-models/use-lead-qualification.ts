...
export function useLeadQualification(leadId: string, onComplete: () => void) {
  ...
  function submit() {
    ...
    busy.current = true
    try {
      const request: QualificationDecision = decision === 'approved' ? { decision, createOpportunity } : { decision, reason, note }
  ...
  return { decision, setDecision, createOpportunity, setCreateOpportunity, reason, setReason, note, setNote, error, submit }
}
