...
type SalesDraft = Omit<Lead, 'status'> &
  Opportunity &
  Omit<Product, 'status'> &
...
export function useSalesEditViewModel(entity: SalesEntity, record?: Lead | Opportunity | Product | Order) {
  const [initial] = useState<SalesDraft>(() => ({
    id: '',
    name: '',
    company: '',
    email: '',
    ...
    opportunityId: '',
    items: [],
    status: entity === 'products' ? '啟用' : entity === 'orders' ? '草稿' : '待聯繫',
    ...record,
  }));
  ...
  function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      (entity === 'opportunities' || entity === 'orders') &&
      !customerRepository.getSnapshot().some((item) => item.id === draft.customerId)
    ) {
      setError('請選擇所屬客戶。');
      return;
    }
    ...
    if (entity === 'leads') {
      ...
      leadRepository.save({
        ...
        source,
        owner,
        status: current?.qualification ? current.status : (draft.status as Lead['status']),
      });
      ...
      if (
        productRepository
          .getSnapshot()
          .some((item) => item.id !== id && item.sku.toLowerCase() === draft.sku.trim().toLowerCase())
      ) {
        setError('產品編號已存在。');
        return;
      }
      ...
      if (
        ...
        draft.items.some(
          (line) =>
            !productRepository.getSnapshot().some((p) => p.id === line.productId) ||
        ...
        setError('請加入產品，數量須為正整數，單價須為非負數。');
        return;
      }
      if (
        draft.opportunityId &&
        !opportunityRepository
          .getSnapshot()
          .some((item) => item.id === draft.opportunityId && item.customerId === customerId)
      ) {
        setError('來源商機必須屬於所選客戶。');
        return;
      }
  ...
  return {
    ...
    update: <K extends keyof SalesDraft>(field: K, value: SalesDraft[K]) => {
