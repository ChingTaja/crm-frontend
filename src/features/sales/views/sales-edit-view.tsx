...
export function SalesEditView({ vm, record }: { vm: SalesViewModel; record?: Lead | Opportunity | Product | Order }) {
  ...
  const textFields =
    vm.entity === 'leads'
      ? (['company', 'email', 'phone', 'source', 'owner'] as const)
      : vm.entity === 'opportunities'
        ? (['owner', 'expectedCloseDate'] as const)
        : vm.entity === 'products'
          ? (['sku'] as const)
          : [];
  const labels = {
    ...
    <>
      <CustomerPageHeader>
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" aria-label="返回列表" onClick={form.back}>
            <ArrowLeft />
          </Button>
          <CustomerPageTitle>
            {record ? '編輯' : '新增'}
          ...
          {lead && <LeadQualificationDialog lead={lead} disabled={form.isDirty} />}
        </div>
      ...
      <form id="sales-form" onSubmit={form.save} className="mx-auto max-w-5xl space-y-6 py-8">
        <Card>
          <CardHeader>
            <CardTitle>{vm.title}基本資料</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-6 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="sales-name">名稱 *</Label>
              <Input id="sales-name" required value={draft.name} onChange={(e) => update('name', e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="sales-status">{vm.entity === 'opportunities' ? '階段' : '狀態'}</Label>
              <select
                disabled={!!lead?.qualification}
                id="sales-status"
                className="h-9 w-full rounded-lg border px-3 text-sm"
                value={vm.entity === 'opportunities' ? draft.stage : draft.status}
                onChange={(e) =>
                  vm.entity === 'opportunities'
                    ? update('stage', e.target.value as Opportunity['stage'])
            ...
            {relatedCustomer && (
              <div className="space-y-2">
                <Label htmlFor="sales-customer">所屬客戶 *</Label>
                <Lookup
                  id="sales-customer"
                  label="所屬客戶"
                  required
            ...
            {vm.entity === 'opportunities' && (
              <div className="space-y-2">
                <Label htmlFor="sales-lead">來源 Lead</Label>
                <Lookup
                  id="sales-lead"
                  label="來源 Lead"
                  value={draft.leadId}
                  options={[{ value: '', label: '無' }, ...vm.leads.map((l) => ({ value: l.id, label: l.name }))]}
            ...
            {vm.entity === 'orders' && (
              <div className="space-y-2">
                <Label htmlFor="sales-opportunity">來源商機</Label>
                <Lookup
                  ...
                  onValueChange={(value) => update('opportunityId', value)}
                />
              </div>
            )}
            {(vm.entity === 'opportunities' || vm.entity === 'products') && (
              <div className="space-y-2">
                <Label htmlFor="sales-price">{vm.entity === 'products' ? '單價' : '預估金額'}（TWD）</Label>
                <Input
                  id="sales-price"
                  type="number"
                  required
                  min="0"
                  step="0.01"
                  value={vm.entity === 'products' ? draft.price : draft.amount}
                  onChange={(e) => update(vm.entity === 'products' ? 'price' : 'amount', e.target.valueAsNumber)}
                />
              </div>
            )}
          </CardContent>
                          ...
                          draft.items.map((item, i) =>
                            i === index
                              ? {
                                  ...item,
                                  productId: value,
                                  unitPrice: vm.products.find((p) => p.id === value)?.price ?? 0,
                                }
                              : item
                          )
                        )
