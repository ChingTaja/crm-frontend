...
import { canEditQuote, quoteToday } from '../models/quote-policy'
...
export function useQuoteEditor(actor: QuoteActor, quote?: Quote, version?: QuoteVersion) {
  ...
  const editable = actor.role === 'sales' && latest && (!version || canEditQuote(version))
  ...
  function selectProduct(id: string, product: Product) {
    setDraft(current => ({ ...current, lines: current.lines.map(line => line.id === id ? { ...line, productId: product.id, productName: product.name, sku: product.sku, catalogPrice: product.price, unitPrice: product.price } : line) }))
  }
  ...
  return { draft, error, notice, setNotice, editable, latest, dirty, update, updateLine, selectProduct, run, save,
