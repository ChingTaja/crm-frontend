...
export function QuoteEditorView({ vm, quote, version, onVersion }: { vm: QuoteViewModel; quote?: Quote; version?: QuoteVersion; onVersion: (id: string) => void }) {
  ...
  return <>
    <CustomerPageHeader>
      <div className="flex items-center gap-2"><Button variant="ghost" size="icon" aria-label="返回報價單" onClick={() => { window.location.hash = '/quotes' }}><ArrowLeft /></Button><CustomerPageTitle>{quote ? `${quote.number} · v${version?.version}` : '新增報價單'}</CustomerPageTitle></div>
    ...
    <div className="mx-auto max-w-6xl space-y-5 py-6">
      {quote && version && <div className="flex flex-wrap items-center gap-3 text-sm">
        <Label htmlFor="quote-version">版本</Label><select id="quote-version" className="h-9 rounded-lg border px-3" disabled={editor.dirty} value={version.id} onChange={event => onVersion(event.target.value)}>{quote.versions.map(v => <option key={v.id} value={v.id}>v{v.version} · {v.status}</option>)}</select>
      ...
      {quote && version && <QuoteActions quote={quote} version={version} actor={vm.actor} dirty={editor.dirty} onVersion={onVersion} />}
