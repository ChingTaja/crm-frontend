...
import type { Order } from '@/features/sales/models/sales-model'
import { QuoteTotals } from '../components/quote-lines'
import { money, quoteLineTotals } from '../models/quote-policy'

export function QuoteOrderView({ order }: { order: Order }) {
  const source = order.quoteSource!
  return <>
    <CustomerPageHeader><CustomerPageTitle>{order.name}</CustomerPageTitle><Button variant="outline" onClick={() => { window.location.hash = '/orders' }}>返回訂單</Button></CustomerPageHeader>
