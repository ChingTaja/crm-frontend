...
export function OrderView({ recordId }: { recordId?: string }) {
  ...
  return <EntityWorkspace sidebar={<SalesSidebar entity="orders" />}>
    {recordId ? record?.quoteSource ? <QuoteOrderView order={record} /> : recordId === 'new' || record ? <OrderEditView key={recordId} record={record} /> : <p className="py-10">找不到資料。<a className="underline" href="#/orders">返回列表</a></p> : <EntityList vm={vm} />}
  </EntityWorkspace>
}
