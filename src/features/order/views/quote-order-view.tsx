...
export function QuoteOrderView({ order }: { order: Order }) {
  ...
  return <>
    <EntityPageHeader><EntityPageTitle>{order.name}</EntityPageTitle><Button variant="outline" onClick={() => { navigate('/orders') }}>返回訂單</Button></EntityPageHeader>
    <div className="mx-auto max-w-5xl space-y-5 py-6">
      <p className="rounded-lg bg-muted p-3 text-sm">來源：<AppLink className="underline" href={`/quotes/${source.quoteId}/edit`}>{source.number} v{source.version}</AppLink>。此訂單保留已接受報價的明細、折扣、稅金及條款快照，目前為唯讀。</p>
