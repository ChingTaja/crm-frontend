...
function LoginView({ viewModel }: { viewModel: LoginViewModel }) {
  const { credentials, showPassword, notice, isSubmitting, updateField, submit, togglePassword, requestPasswordReset } =
    viewModel;
  return (
    <main className="grid min-h-svh grid-cols-1 bg-white text-[#263d37] min-[761px]:grid-cols-[47%_53%]">
      ...
      <section
        className="flex min-h-[610px] min-w-0 flex-col px-6 py-6 min-[761px]:px-8 min-[761px]:pt-8 min-[1001px]:px-12 min-[1001px]:pt-10"
      ...
      >
        <div className="m-auto w-full max-w-[400px] py-8 min-[761px]:max-w-[370px] min-[761px]:pt-15 min-[761px]:pb-10 min-[1500px]:max-w-[400px]">
          ...
          <h2 id="login-title" className="mt-3 mb-2.5 text-[28px] font-semibold tracking-wide min-[761px]:text-[31px]">
            歡迎回來
          </h2>
          ...
          <form onSubmit={submit} aria-busy={isSubmitting}>
            ...
            <div className="mt-6 mb-2 flex items-center justify-between">
              ...
              <Button
                variant="link"
                className="h-auto px-0 py-1 text-[11px] text-[#3b6550]"
                type="button"
            ...
            <Button
              className="h-12 w-full cursor-pointer gap-4 rounded-lg bg-[#254e3c] text-[13px] text-white shadow-sm hover:bg-[#193d2d] active:translate-y-px disabled:cursor-wait disabled:opacity-60"
              type="submit"
              disabled={isSubmitting}
            >
