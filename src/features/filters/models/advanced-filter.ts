export const filterOperators = {
  contains: '包含', notContains: '不包含', equals: '等於', notEquals: '不等於',
  startsWith: '開頭是', empty: '為空', notEmpty: '不為空',
} as const
...
export const createFilterGroup = (): FilterGroup => ({ kind: 'group', id: crypto.randomUUID(), match: 'all', children: [] })
export const createFilterRule = (): FilterRule => ({ kind: 'rule', id: crypto.randomUUID(), field: 0, operator: 'contains', value: '' })
...
export function matchesAdvancedFilter(values: string[], node: FilterNode): boolean {
  ...
  switch (node.operator) {
    ...
    case 'empty': return actual === '' || actual === '—'
    case 'notEmpty': return actual !== '' && actual !== '—'
  }
}
