...
export function OpportunityView({ recordId }: { recordId?: string }) {
  ...
  return <EntityWorkspace sidebar={<SalesSidebar entity="opportunities" />}>
    {recordId ? recordId === 'new' || record ? <OpportunityEditView key={recordId} record={record} /> : <p className="py-10">找不到資料。<AppLink className="underline" href="/opportunities">返回列表</AppLink></p> : <EntityList vm={vm} />}
  </EntityWorkspace>
}
