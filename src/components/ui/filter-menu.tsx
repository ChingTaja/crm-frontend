    ...
    >
      ...
      <PopoverContent className="w-80">
        ...
        <div className="flex items-center gap-2 border-b px-3 py-2.5">
          {draft ? (
            <Button variant="ghost" size="icon-sm" aria-label="返回欄位列表" onClick={() => setDraft(null)}>
              <ArrowLeft />
            </Button>
          ) : (
            <PopoverClose render={<Button variant="ghost" size="icon-sm" aria-label="關閉篩選" />}>
              <X />
            </PopoverClose>
          )}
        ...
        {draft ? (
          <>
            <div className="relative border-b">
              ...
              >
                {getFieldOperators(fields[draft.field]).map((key) => (
            ...
            {requiresFilterValue(draft.operator) ? (
              <div className="p-3">
                <FilterValue
                  ...
                  value={draft.value}
                  onChange={(next) => updateDraft({ ...draft, value: next })}
                />
              </div>
            ) : (
          ...
          <>
            ...
            <div className="max-h-96 overflow-y-auto">
              {[false, true].map((hidden) => {
                const matching = fieldOrder
                  .map((index) => ({ ...fields[index], index }))
                  .filter(
                    (field) => hiddenFields.includes(field.index) === hidden && field.label.includes(search.trim())
                  );
                return (
                  <section key={String(hidden)} aria-label={hidden ? '隱藏欄位' : '可見欄位'}>
                    <h3 className="bg-muted/50 px-4 py-2 text-xs text-muted-foreground">
                      {hidden ? '隱藏欄位' : '可見欄位'}
                    </h3>
                    <div className="p-2">
                      {matching.map(({ label, hideable = true, index }) => (
                        <div
                          key={label}
                          className="flex items-center gap-1 rounded-md data-[target=true]:bg-blue-50 data-[target=true]:ring-1 data-[target=true]:ring-blue-300"
                          data-target={dropTarget === index}
                          onDragOver={(event) => {
                        ...
                        >
                          <Button
                            ...
                            draggable
                            aria-label={`拖移${label}欄位，也可使用上下方向鍵排序`}
                            title="拖曳排序，或使用上下方向鍵"
                            onDragStart={(event) => {
                              setDragged(index);
                            ...
                            onKeyDown={(event) => {
                              ...
                              if (target) {
                                onMoveField(index, target.index);
                                setAnnouncement(`已${event.key === 'ArrowUp' ? '上移' : '下移'}${label}`);
                              }
                            }}
                          >
                            <GripVertical />
                          ...
                          <Button
                            variant="ghost"
                            className="h-11 min-w-0 flex-1 justify-start gap-3 text-muted-foreground"
                            onClick={() =>
                              setDraft(
                                value?.field === index
                                  ? { ...value }
                                  : { field: index, operator: defaultFieldOperator(fields[index]), value: '' }
                              )
                            }
                          >
                            <FilterFieldIcon field={fields[index]} />
                            {label}
                          </Button>
                          <Button
                            type="button"
                            ...
                            size="icon-sm"
                            disabled={!hideable}
                            aria-label={hideable ? `${hidden ? '顯示' : '隱藏'}${label}欄位` : `${label}欄位不可隱藏`}
                      ...
                      {!matching.length && (
                        <p className="px-2 py-3 text-xs text-muted-foreground">
                          {search.trim() ? '找不到符合的欄位' : hidden ? '目前沒有隱藏欄位' : '目前沒有可見欄位'}
                        </p>
                      )}
                    </div>
                  </section>
